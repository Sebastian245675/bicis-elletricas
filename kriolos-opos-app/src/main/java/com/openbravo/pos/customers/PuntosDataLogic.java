...
public class PuntosDataLogic {
    ...
    public void initTables() throws BasicException {
        ...
        try {
            ...
            // Sebastian - Crear tabla de puntos redimidos si no existe
            String createRedimidosTable = "CREATE TABLE PUNTOS_REDIMIDOS (" +
                    "ID VARCHAR(36) NOT NULL PRIMARY KEY, " +
                    "CLIENTE_ID VARCHAR(36) NOT NULL, " +
            ...
            try {
                new StaticSentence(s, createRedimidosTable).exec();
                System.out.println("✅ Tabla PUNTOS_REDIMIDOS creada exitosamente");
            } catch (BasicException e) {
                if (e.getMessage().contains("already exists") || e.getMessage().contains("ya existe") ||
                        e.getMessage().contains("nombre del objeto ya existe")) {
                    System.out.println("ℹ️ Tabla PUNTOS_REDIMIDOS ya existe");
                } else {
                    System.err.println("⚠️ Error creando tabla PUNTOS_REDIMIDOS: " + e.getMessage());
                }
            }
