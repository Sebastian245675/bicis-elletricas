    ...
    private void confirmarCierre() {
        try {
            ...
            if (m_App.getActiveCashDateEnd() == null) {
                try {
                    // Intento 1: Con columnas extendidas (faltante/sobrante)
                    ...
                    // Intento 2: Fallback a SQL estándar si las columnas no existen
                    LOGGER.log(Level.INFO, "Columnas faltante/sobrante no encontradas, usando SQL estándar.");
                    StaticSentence<Object[], Integer> stmt = new StaticSentence<>(
                            m_App.getSession(),
                            "UPDATE closedcash SET DATEEND = ?, NOSALES = ? WHERE HOST = ? AND MONEY = ?",
                            new SerializerWriteBasic(new Datas[] {
                                    Datas.TIMESTAMP, Datas.INT, Datas.STRING, Datas.STRING }));
                    stmt.exec(new Object[] { dNow, m_PaymentsToClose.getSales(),
                            m_App.getProperties().getHost(), oldCashIndex });
                }
            }
