...
public class JRootMenu {
    ...
    public void setRootMenu(JScrollPane m_jPanelLeft, DataLogicSystem m_dlSystem) {
        try {
            ...
            String bundledMenu = StringUtils.readResource(pah);

            // Existing databases may still have an older cached Menu.Root without
            // recently added entries like Recursos Humanos. In that case prefer the
            // bundled template so the new option is visible without manual SQL.
            if (shouldUseBundledRootMenu(menuScrip, bundledMenu)) {
                LOGGER.log(Level.INFO, "Using bundled Menu.Root because the database copy is outdated.");
                menuScrip = bundledMenu;
            }
            ...
            if (menuComponent != null) {
                ...
                menuComponent = getScriptMenu(bundledMenu);
                if (menuComponent != null) {
                    m_jPanelLeft.setViewportView(menuComponent);
                } else {
    ...
    public class ScriptGroup implements Menu.MenuGroup{
        ...
        private ScriptGroup(String key, Component _parent, AppUserView _appview) {
            ...
            taskGroup.setRequestFocusEnabled(false);
                        String titleVal = AppLocal.getIntString(key); taskGroup.setTitle(stripTags(titleVal == null || titleVal.isBlank() ? key : titleVal));
    ...
    public class ScriptSubmenu implements Menu.Submenu{
        ...
        public void addTitle(String key) {
                        String titleStr = AppLocal.getIntString(key); menudef.addMenuTitle(stripTags(titleStr == null || titleStr.isBlank() ? key : titleStr));
        }
