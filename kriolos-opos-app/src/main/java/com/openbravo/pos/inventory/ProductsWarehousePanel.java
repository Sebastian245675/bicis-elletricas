...
public class ProductsWarehousePanel extends JPanelTable2 {
    ...
    protected void init() {   
        ...
        row = new Row(
                ...
                new Field("LOCATION_NAME", Datas.STRING, Formats.STRING),
                new Field("LOCATION_ADDRESS", Datas.STRING, Formats.STRING),
                new Field("LOCATION_PHONE", Datas.STRING, Formats.STRING),
        ...
        lpr = new ListProviderCreator(new PreparedSentence(app.getSession(),
            "SELECT L.ID, P.ID, P.REFERENCE, P.NAME, " +
                "LOC.ID AS LOCID, LOC.NAME AS LOCNAME, LOC.ADDRESS AS LOCADDR, LOC.PHONE AS LOCPHONE, " +
                "L.STOCKSECURITY, L.STOCKMAXIMUM, COALESCE(S.SUMUNITS, 0) " +
                "FROM products P " +
                "CROSS JOIN locations LOC " +
                "LEFT OUTER JOIN stocklevel L ON P.ID = L.PRODUCT AND L.LOCATION = LOC.ID " +
        ...
        SentenceExec updatesent =  new SentenceExecTransaction(app.getSession()) {
            ...
            public int execInTransaction(Object[] params) throws BasicException {
                ...
                Double newQuantity = (Double) values[10];
                ...
                if (values[0] == null)  {
                    ...
                    result = new PreparedSentence(app.getSession()
                        ...
                        , new SerializerWriteBasicExt(row.getDatas(), new int[] {0, 4, 1, 8, 9})).exec(params);
                } else {
                    // UPDATE
                    result = new PreparedSentence(app.getSession()
                        , "UPDATE stocklevel SET STOCKSECURITY = ?, STOCKMAXIMUM = ? WHERE ID = ?"
                        , new SerializerWriteBasicExt(row.getDatas(), new int[] {8, 9, 0})).exec(params);
                }
