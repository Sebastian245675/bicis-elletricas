...
public class DefaultRolesInitializer {
    ...
    private static void initializeRole(Session session, String id, String name, Set<String> permissions)
        ...
        StaticSentence<Object[], Object[]> checkRoleById = new StaticSentence<>(session,
                "SELECT id, name FROM roles WHERE id = ?",
                new SerializerWriteBasic(new Datas[] { Datas.STRING }),
                new SerializerReadBasic(new Datas[] { Datas.STRING, Datas.STRING }));

        Object resultRowById = checkRoleById.find(new Object[] { id });
        ...
        // antiguas)
        if (!roleExists) {
            StaticSentence<Object[], Object[]> checkRoleByName = new StaticSentence<>(session,
                    "SELECT id, name FROM roles WHERE UPPER(name) = UPPER(?)",
                    new SerializerWriteBasic(new Datas[] { Datas.STRING }),
                    new SerializerReadBasic(new Datas[] { Datas.STRING, Datas.STRING }));

            Object resultRowByName = checkRoleByName.find(new Object[] { name });
        ...
        if (!roleExists) {
            ...
            PreparedSentence<Object[], Object> updateRole = new PreparedSentence<>(session,
    ...
    private static void initializeRoleIfNotExists(Session session, String id, String name, Set<String> permissions)
        ...
        StaticSentence<Object[], Object[]> checkRole = new StaticSentence<>(session,
                "SELECT id FROM roles WHERE name = ?",
                new SerializerWriteBasic(new Datas[] { Datas.STRING }),
                new SerializerReadBasic(new Datas[] { Datas.STRING }));

        Object resultRow = checkRole.find(new Object[] { name });
    ...
    private static Set<String> getManagerPermissions() {
        ...
        permissions.add("com.openbravo.pos.inventory.JPanelPromotions");
        permissions.add("com.openbravo.pos.inventory.JPanelProductVariation");
