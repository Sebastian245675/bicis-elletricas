...
public final class JRViewer400 extends javax.swing.JPanel implements JRHyperlinkListener {
    ...
    private void applyCorporateViewerStyle() {
        Color canvas = new Color(243, 245, 248);
        Color surface = Color.WHITE;
        Color toolbarSurface = new Color(247, 249, 251);
        Color border = new Color(214, 222, 230);
        Color text = new Color(70, 82, 96);

        setBackground(surface);

        jToolBar.setFloatable(false);
        jToolBar.setRollover(true);
        jToolBar.setBackground(toolbarSurface);
        ...
        for (Component component : jToolBar.getComponents()) {
            styleToolbarComponent(component, surface, toolbarSurface, border, text);
        }

        pnlMain.setBackground(canvas);
        pnlInScroll.setBackground(canvas);
        pnlPage.setBackground(canvas);
        jPanel4.setBackground(canvas);
        jPanel5.setBackground(canvas);
        jPanel6.setBackground(canvas);
        jPanel7.setBackground(canvas);
        jPanel8.setBackground(canvas);
        jPanel9.setBackground(canvas);
        pnlTabs.setBackground(surface);

        scrollPane.setBorder(BorderFactory.createEmptyBorder());
        scrollPane.setBackground(canvas);
        scrollPane.getViewport().setBackground(canvas);

        lblPage.setBackground(surface);
        lblPage.setBorder(BorderFactory.createLineBorder(border));

        pnlStatus.setBackground(surface);
        pnlStatus.setBorder(BorderFactory.createMatteBorder(1, 0, 0, 0, border));
        lblStatus.setForeground(text);
        lblStatus.setBorder(new EmptyBorder(6, 0, 6, 0));
        jLabel1.setText("");
    }

    private void styleToolbarComponent(Component component, Color surface, Color toolbarSurface, Color border, Color text) {
        if (component instanceof JToolBar.Separator) {
            component.setBackground(toolbarSurface);
            component.setForeground(border);
            component.setPreferredSize(new Dimension(14, 30));
            return;
        }
